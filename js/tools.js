...
(function () {
  ...
  function toBin(n, pad) { let s = (n >>> 0).toString(2); while (pad && s.length < pad) s = "0" + s; return s; }

  window.Tools.binario = function (mount) {
    const el = h(`<div>
      <div class="tool">
        <h3 class="mt-0">Decimal → binario</h3>
        <div class="field"><label>Número entero (0–255 muestra 8 bits)</label>
          <input class="input" id="bDec" type="number" value="192"></div>
        <div class="result-box input--mono" id="bDecOut"></div>
      </div>
      <div class="tool">
        <h3 class="mt-0">Binario → decimal</h3>
        <div class="field"><label>Bits (solo 0 y 1)</label>
          <input class="input input--mono" id="bBin" value="11000000"></div>
        <div class="result-box" id="bBinOut"></div>
      </div>
      <div class="tool">
        <h3 class="mt-0">Dirección IP en binario</h3>
        <div class="field"><label>IP (a.b.c.d)</label>
          <input class="input input--mono" id="bIp" value="192.168.0.1"></div>
        <div class="octets" id="bIpOut"></div>
      </div>
    </div>`);
    mount.appendChild(el);

    function dec() {
      const v = parseInt(el.querySelector("#bDec").value, 10);
      const out = el.querySelector("#bDecOut");
      if (isNaN(v) || v < 0) { out.className = "result-box empty input--mono"; out.textContent = "Ingresá un entero ≥ 0."; return; }
      out.className = "result-box input--mono";
      out.textContent = (v >= 0 && v <= 255) ? toBin(v, 8) : toBin(v);
    }
    function bin() {
      const s = String(el.querySelector("#bBin").value).trim();
      const out = el.querySelector("#bBinOut");
      if (!/^[01]+$/.test(s)) { out.className = "result-box empty"; out.textContent = "Usá solo 0 y 1."; return; }
      out.className = "result-box";
      out.innerHTML = `<span class="result-grid"><span class="k">Decimal</span><span class="v">${parseInt(s, 2).toLocaleString("es-AR")}</span></span>`;
    }
    function ip() {
      const out = el.querySelector("#bIpOut");
      const parts = String(el.querySelector("#bIp").value).trim().split(".");
      if (parts.length !== 4 || parts.some(p => !/^\d+$/.test(p) || +p > 255)) {
        out.innerHTML = `<div class="result-box empty" style="flex:1">IP no válida (cada octeto 0–255).</div>`; return;
      }
      out.innerHTML = parts.map(p => `<div class="octet"><div class="bits">${toBin(+p, 8)}</div><div class="dec">${p}</div></div>`).join("");
    }
    el.querySelector("#bDec").addEventListener("input", dec);
    el.querySelector("#bBin").addEventListener("input", bin);
    el.querySelector("#bIp").addEventListener("input", ip);
    dec(); bin(); ip();
  };
})();
